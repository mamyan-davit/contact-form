<?php 
	class Validator{
		...
		private function isErrorKeyValid($key=null){
			return(
				!empty($key) && !array_key_exists($key, $this->errors)
			);
		}

		public function addError($key=null){
			if ($this->isErrorKeyValid($key)) {
		...
		private function _isEmptyAndRequired($key=null, $value=null){
			return(
				self::isEmpty($value)&&in_array($key, $this->required)
			);
		}

		private function isEmailValid($key=null, $value=null){
